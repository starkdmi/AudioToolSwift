...
public class MossFormer2Pipeline {
    ...
    /// Result of background extraction containing both enhanced and background audio
    public struct BackgroundExtractionResult {
        public let enhanced: MLXArray
        public let background: MLXArray
    }
    
    /// Process short audio with background extraction
    /// - Returns: (enhanced, background) MLXArrays
    internal func processShortAudioWithBackground(
        model: TestNet,
        audio: MLXArray,
        args: PipelineConfiguration.Args,
        gamma: Float = 1.10
    ) throws -> BackgroundExtractionResult {
        let scaledAudio = audio * Self.MAX_WAV_VALUE
        let scaledAudioBatch = scaledAudio.reshaped([1, -1])
        ...
        let outList = model(fbanks)
        var predMask = outList.last![0].transposed(1, 0)
        if predMask.ndim == 3 && predMask.shape[2] == 1 {
            predMask = predMask.squeezed(axis: 2)
        }
        
        let window = createWindow(type: args.winType, length: args.winLen)
        let (realPart, imagPart) = stft(scaledAudio,
                                         nFFT: args.fftLen,
                                         hopLength: args.winInc,
                                         winLength: args.winLen,
                                         window: window,
        ...
        var real = realPart, imag = imagPart
        if real.ndim == 3 && real.shape[0] == 1 {
            real = real[0]
            imag = imag[0]
        }
        
        // Enhanced = clipped mask * spectrum
        let clippedMask = MLX.clip(predMask, min: 0.0, max: 1.0)
        let maskedReal = real * clippedMask
        let maskedImag = imag * clippedMask
        
        // Background = pow(1 - mask, gamma) * spectrum
        let backgroundMask = MLX.pow(1.0 - clippedMask, MLXArray(gamma))
        let bgReal = real * backgroundMask
        let bgImag = imag * backgroundMask
        
        let enhanced = istft(
            real: maskedReal.expandedDimensions(axis: 0),
            imag: maskedImag.expandedDimensions(axis: 0),
            nFFT: args.fftLen,
            hopLength: args.winInc,
            winLength: args.winLen,
            window: window,
        ...
        ).squeezed(axis: 0) / Self.MAX_WAV_VALUE
        
        let background = istft(
            real: bgReal.expandedDimensions(axis: 0),
            imag: bgImag.expandedDimensions(axis: 0),
            nFFT: args.fftLen,
            hopLength: args.winInc,
            winLength: args.winLen,
            window: window,
        ...
        return BackgroundExtractionResult(enhanced: enhanced, background: background)
    }
    
    /// Enhance audio array with background extraction
    /// - Parameters:
    ///   - audioArray: Input audio as MLXArray
    ...
    /// - Returns: BackgroundExtractionResult with enhanced and background audio
    public func enhanceAudioWithBackground(_ audioArray: MLXArray, gamma: Float = 1.10) throws -> BackgroundExtractionResult {
        return try processShortAudioWithBackground(
            model: model,
            audio: audioArray,
