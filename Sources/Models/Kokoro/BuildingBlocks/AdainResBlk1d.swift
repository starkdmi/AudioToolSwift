  ...
  init(
    ...
    if upsample == "none" {
      ...
      self._pool.wrappedValue = ConvWeighted(
        weightG: try weights.required(weightKeyPrefix + ".pool.weight_g"),
        weightV: try weights.required(weightKeyPrefix + ".pool.weight_v"),
        bias: try weights.required(weightKeyPrefix + ".pool.bias"),
        stride: 2,
        padding: 1,
        groups: dimIn
      )
    ...
    self._conv1.wrappedValue = ConvWeighted(
      weightG: try weights.required(weightKeyPrefix + ".conv1.weight_g"),
      weightV: try weights.required(weightKeyPrefix + ".conv1.weight_v"),
      bias: try weights.required(weightKeyPrefix + ".conv1.bias"),
      stride: 1,
      padding: 1
    )

    self._conv2.wrappedValue = ConvWeighted(
      weightG: try weights.required(weightKeyPrefix + ".conv2.weight_g"),
      weightV: try weights.required(weightKeyPrefix + ".conv2.weight_v"),
      bias: try weights.required(weightKeyPrefix + ".conv2.bias"),
      stride: 1,
      padding: 1
    )
    ...
    self._norm1.wrappedValue = AdaIN1d(
      styleDim: styleDim,
      numFeatures: dimIn,
      fcWeight: try weights.required(weightKeyPrefix + ".norm1.fc.weight"),
      fcBias: try weights.required(weightKeyPrefix + ".norm1.fc.bias")
    )

    self._norm2.wrappedValue = AdaIN1d(
      styleDim: styleDim,
      numFeatures: dimIn,
      fcWeight: try weights.required(weightKeyPrefix + ".norm2.fc.weight"),
      fcBias: try weights.required(weightKeyPrefix + ".norm2.fc.bias")
    )

    if learned_sc {
      self._conv1x1.wrappedValue = ConvWeighted(
        weightG: try weights.required(weightKeyPrefix + ".conv1x1.weight_g"),
        weightV: try weights.required(weightKeyPrefix + ".conv1x1.weight_v"),
        bias: nil,
        stride: 1,
        padding: 0
      )
