...
final class ProsodyPredictor: Module {
  ...
  public init(weights: [String: MLXArray], styleDim: Int, dHid: Int) throws {
    ...
    self._shared.wrappedValue = LSTM(
      ...
      wxForward: try weights.required("predictor.shared.weight_ih_l0"),
      whForward: try weights.required("predictor.shared.weight_hh_l0"),
      biasIhForward: try weights.required("predictor.shared.bias_ih_l0"),
      biasHhForward: try weights.required("predictor.shared.bias_hh_l0"),
      wxBackward: try weights.required("predictor.shared.weight_ih_l0_reverse"),
      whBackward: try weights.required("predictor.shared.weight_hh_l0_reverse"),
      biasIhBackward: try weights.required("predictor.shared.bias_ih_l0_reverse"),
      biasHhBackward: try weights.required("predictor.shared.bias_hh_l0_reverse")
    )
    ...
    self._F0Proj.wrappedValue = Conv1dInference(
      ...
      padding: 0,
      weight: try weights.required("predictor.F0_proj.weight"),
      bias: try weights.required("predictor.F0_proj.bias")
    )
    ...
    self._NProj.wrappedValue = Conv1dInference(
      ...
      kernelSize: 1,  // 1x1 convolution for channel reduction
      padding: 0,
      weight: try weights.required("predictor.N_proj.weight"),
      bias: try weights.required("predictor.N_proj.bias")
    )
  }
