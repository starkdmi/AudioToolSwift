...
class Generator: Module {
       ...
       genIstftHopSize: Int) throws
  {
    ...
    for (i, (u, k)) in zip(upsampleRates, upsampleKernelSizes).enumerated() {
      upsArr.append(
        ConvWeighted(
          weightG: try KokoroWeights.require(weights, "decoder.generator.ups.\(i).weight_g"),
          weightV: try KokoroWeights.require(weights, "decoder.generator.ups.\(i).weight_v"),
          bias: try KokoroWeights.require(weights, "decoder.generator.ups.\(i).bias"),
          stride: u,
          padding: (k - u) / 2
        )
      )
      ...
      if i + 1 < upsampleRates.count {
        ...
        noiseConvsArr.append(
          Conv1dInference(
            ...
            weight: try KokoroWeights.require(weights, "decoder.generator.noise_convs.\(i).weight"),
            bias: try KokoroWeights.require(weights, "decoder.generator.noise_convs.\(i).bias")
          )
        )
    ...
    self._convPost.wrappedValue = ConvWeighted(
      weightG: try KokoroWeights.require(weights, "decoder.generator.conv_post.weight_g"),
      weightV: try KokoroWeights.require(weights, "decoder.generator.conv_post.weight_v"),
      bias: try KokoroWeights.require(weights, "decoder.generator.conv_post.bias"),
      stride: 1,
      padding: 3
    )
