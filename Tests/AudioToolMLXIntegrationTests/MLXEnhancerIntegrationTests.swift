...
struct TestConfig {
    // Weights, fixtures and reference audio all live in the sibling research
    // checkout, which no standalone clone has. Absent that, these resolve to nil
    // and the requiring test reports what is missing instead of dereferencing a
    // path that was never there.

    static let frcrn16kWeights =
        TestGate.reference("Models/frcrn_se_mlx_swift/Weights/frcrn_se_16k.safetensors")?.path
    ...
    static let frcrnTestAudio = TestGate.reference("Models/frcrn_se_mlx_swift/test.wav")?.path
    static let mossformer2TestAudio = TestGate.reference("Models/mossformer2_se_mlx_swift/test.wav")?.path

    /// Where enhanced audio is written for listening. Scratch: these are artifacts
    /// of a test run, and writing them back into the model directories overwrote
    /// files that belong to those directories.
    static func outputDirectory(_ name: String) throws -> String {
        try TestGate.outputDirectory(name).path
    }

    /// Whether MLX integration tests should run.
    ///
    /// Two gates, not one. `SKIP_MLX_TESTS=1` alone was never enough: it defaulted
    /// to *running* real models against files outside the package, so a fresh clone
    ...
    static var isCI: Bool { TestGate.isCI }
...
extension Trait where Self == Testing.ConditionTrait {
    ...
    static var enabledForMLX: Self {
        .enabled(if: TestConfig.shouldRunIntegrationTests,
                 "MLX integration tests - set RUN_INTEGRATION_TESTS=1 (and do not set SKIP_MLX_TESTS)")
    }
}
...
struct FRCRNChunkingIntegrationTests {
    ...
    func testFRCRNWithChunking() async throws {
        let frcrn16kWeights = try #require(TestConfig.frcrn16kWeights, "FRCRN weights not found in the sibling research checkout")
        let frcrnTestAudio = try #require(TestConfig.frcrnTestAudio, "FRCRN test audio not found in the sibling research checkout")
        
        print("\n=== FRCRN SE 16K with Chunking ===")
        
        // Create provider (chunking is auto-enabled)
        let provider = FRCRNSE16KProvider(weightsPath: frcrn16kWeights)
        ...
        let testAudioPath = frcrnTestAudio
        ...
        // Save output
        let outputPath = try TestConfig.outputDirectory("FRCRN") + "/enhanced_6s_chunked.wav"
        let saverConfig = AudioSaver.Configuration(sampleRate: Double(sampleRate))
        let saver = AudioSaver(config: saverConfig)
...
struct MossFormer2ChunkingIntegrationTests {
    ...
    func testMossFormer2SEWithChunking() async throws {
        ...
        // Load test audio
        let mossformer2TestAudio = try #require(TestConfig.mossformer2TestAudio, "MossFormer2 SE test audio not found in the sibling research checkout")
        let loaderConfig = AudioLoader.Configuration(targetSampleRate: 48000)
        let loader = AudioLoader(config: loaderConfig)
        let audio = try loader.loadMono(from: URL(fileURLWithPath: mossformer2TestAudio))
        eval(audio)
        let samples = audio.asArray(Float.self)
        ...
        // Save output
        let outputPath = try TestConfig.outputDirectory("MossFormer2SE") + "/enhanced_test_chunked.wav"
...
struct StreamingVerificationTests {
    ...
    func testFRCRNStreamingQuality() async throws {
        ...
        let outputDir = try TestConfig.outputDirectory("FRCRNStreaming")
        let batchPath = "\(outputDir)/streaming_test_batch.wav"
        let streamPath = "\(outputDir)/streaming_test_stream.wav"
...
struct MossFormer2SSIntegrationTests {
    ...
    func testMossFormer2SS2Speaker() async throws {
        ...
        let testPath = try #require(TestGate.reference("Models/mosforrmer2_ss_mlx_swift/mix.wav")?.path, "2-speaker mix not found in the sibling research checkout")
        ...
        let outputDir = try TestGate.outputDirectory("MossFormer2SS").path
        for (i, output) in outputs.enumerated() {
            let path = "\(outputDir)/ss_2spk_speaker\(i + 1).wav"
    ...
    func testMossFormer2SS3Speaker() async throws {
        ...
        let testPath = try #require(TestGate.reference("Models/mosforrmer2_ss_mlx_swift/mix3_8k.wav")?.path, "3-speaker mix not found in the sibling research checkout")
        print("Loading audio from: \(testPath)")
        ...
        let outputDir = try TestGate.outputDirectory("MossFormer2SS").path
        for (i, output) in outputs.enumerated() {
            let path = "\(outputDir)/ss_3spk_speaker\(i + 1).wav"
            try saver.save(MLXArray(output.samples), to: path)
            print("Saved: \(path)")
        }
...
struct MossFormer2SRIntegrationTests {
    ...
    func testMossFormer2SR48K() async throws {
        ...
        let testPath = try #require(TestGate.reference("Models/mossformer2_sr_mlx_swift/test_16k.wav")?.path, "SR test audio not found in the sibling research checkout")
        print("Loading audio from: \(testPath)")
        ...
        // Save output
        let outputPath = try TestGate.outputDirectory("MossFormer2SR").appendingPathComponent("sr_enhanced_test.wav").path
        let saverConfig = AudioSaver.Configuration(sampleRate: 48000)
        let saver = AudioSaver(config: saverConfig)
        try saver.save(MLXArray(output.samples), to: outputPath)
        print("Saved: \(outputPath)")
